	...
	if (!isset($_SESSION['usua_aif']))
	{
		...
		$imagen_subida = $dir_destino.'/3/'.$name;
		...
		if (!file_exists($dir_destino.'/3/')) {
			mkdir($dir_destino.'/3/', 0777);
		}

		//borro el archivo anterior
		if ($archivoAnterior != '') {
			unlink($dir_destino.'/3/'.$archivoAnterior);
		}
		...
		if (move_uploaded_file($templocation, $imagen_subida)) {
			...
			$res1 = $serviciosArbitros->modificarObservacionesPLanilla($idfixture,'imagen3',$name);
			$res2 = $serviciosArbitros->modificarObservacionesPLanilla($idfixture,'type3',$type);
